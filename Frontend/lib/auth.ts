...
export async function googleSignIn(idToken: string): Promise<AuthResponse & { onboarding_required?: boolean }> {
  if (!AUTH_API_URL) {
    // Dev fallback: no backend – return a mock token so UI flow works
    if (typeof window !== 'undefined' && process.env.NODE_ENV === 'development') {
      return { token: `dev-google-${idToken.slice(0, 12)}`, success: true, onboarding_required: true };
    }
...
export async function registerWithEmail(
  ...
  password: string,
  fullName?: string
): Promise<AuthResponse & { token?: string; onboarding_required?: boolean }> {
  if (!AUTH_API_URL) {
    if (typeof window !== 'undefined' && process.env.NODE_ENV === 'development' && email && username && password) {
      return { token: `dev-email-${btoa(email).slice(0, 12)}`, success: true, onboarding_required: true };
    }
    return { success: false, message: 'Auth API not configured' };
  }
  try {
...
export async function signInWithEmail(
  email: string,
  password: string
): Promise<AuthResponse & { onboarding_required?: boolean }> {
  if (!AUTH_API_URL) {
    if (typeof window !== 'undefined' && process.env.NODE_ENV === 'development') {
...
export async function verifySession(token: string): Promise<AuthResponse & { onboarding_required?: boolean; user?: any }> {
  if (!AUTH_API_URL) {
    return { success: false, message: 'Auth API not configured' };
  }

  try {
    const res = await fetch(`${AUTH_API_URL.replace(/\/$/, '')}/auth/verify`, {
      method: 'GET',
      headers: {
        'Authorization': `Bearer ${token}`,
      },
