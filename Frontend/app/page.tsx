...
const ImpromptuCard = React.memo(function ImpromptuCard_({
  ...
  return (
    <article
      id={`card-${rec.id}`}
      className={`flex flex-col h-full min-h-0 border rounded-2xl p-4 md:p-5 shadow-sm scroll-mt-4 transition-shadow min-w-0 ${cardBg} ${isHighlighted ? 'card-highlight-pulse ring-2 ring-visa-blue ring-offset-2' : ''
        }`}
    >
      <div className="flex flex-col flex-grow min-h-0 overflow-hidden">
        <div className="flex justify-between items-start mb-2 md:mb-3">
          <span
            className={`trust-badge text-[10px] md:text-xs font-bold px-2 py-0.5 md:px-2.5 md:py-1 rounded border flex items-center gap-1 uppercase ${isDarkMode
      ...
      <button
        ...
        className={`font-heading w-full py-3 md:py-4 rounded-xl font-bold text-xs md:text-sm tracking-tight transition-all shrink-0 mt-auto ${isSelected
    ...
    >
      ...
      <div className="flex flex-col flex-grow min-h-0 overflow-hidden">
        <div className="flex justify-between items-start mb-1">
          <span
            className={`trust-badge text-[10px] md:text-xs font-bold px-2 py-0.5 rounded border flex items-center gap-1 w-fit uppercase ${isDarkMode
              ? 'bg-emerald-500/25 text-emerald-300 border-emerald-500/50'
              : 'bg-emerald-50 text-emerald-700 border-emerald-100'
              }`}
          >
      ...
      <div className="shrink-0">
        <button
          type="button"
          onClick={() => onAuthorize(rec.id)}
          className={`font-heading w-full py-3 md:py-4 rounded-xl font-bold text-xs md:text-sm tracking-tight transition-all ${isSelected
            ? 'bg-visa-gold text-slate-900 hover:bg-visa-gold/90'
            : 'bg-visa-blue text-white hover:bg-visa-blue-dark'
            }`}
        >
    ...
    >
      ...
      <AnimatePresence>
        {viewState === 'IDLE' && (
          ...
          >
            {/* Header */}
            <div className={`px-4 py-3 flex items-center gap-3 border-b ${drawerBorder}`}>
            ...
            <div className="p-4 space-y-3">
              ...
              {/* Location */}
              <div>
                <label className={`block text-xs font-semibold mb-1.5 ${drawerText}`}>
                  Location
                </label>
                <input
                  type="text"
              ...
              <div>
                ...
                <div className="relative flex items-center gap-2">
                  ...
                  <button
                    ...
                    disabled={isLoading}
                    className={`absolute right-2 p-2 bg-visa-blue text-white rounded-xl active:scale-95 transition-transform ${isLoading ? 'opacity-50 cursor-not-allowed' : ''
                      }`}
                  >
          ...
          >
            <div className="max-w-md mx-auto px-6 space-y-8">
              <div className="flex items-center gap-4">
                ...
                />
                <div>
                  <h3 className={`text-2xl font-bold tracking-tight ${drawerText}`}>
      ...
      <AnimatePresence>
        {viewState === 'RESULTS' && (
          ...
          >
            {/* Header */}
            <div className={`px-4 py-3 flex items-center justify-between border-b ${drawerBorder} shrink-0`}>
              ...
              <button
                ...
                className={`p-1.5 rounded-full ${isDarkMode ? 'bg-slate-700 text-slate-300 hover:bg-slate-600' : 'bg-slate-100 text-slate-400 hover:bg-slate-200'
                  }`}
              >
                <X className="w-4 h-4" />
              </button>
            ...
            {/* Mode Toggle */}
            <div className={`px-4 py-3 flex items-center justify-between gap-3 border-b ${drawerBorder} shrink-0`}>
              ...
              >
                <button
                  ...
                  className={`font-heading px-3 py-1.5 rounded-lg text-xs font-bold tracking-tight transition-all ${displayMode === 'single'
                ...
                <button
                  ...
                  className={`font-heading px-3 py-1.5 rounded-lg text-xs font-bold tracking-tight transition-all ${displayMode === 'itinerary'
                    ? 'bg-white text-visa-blue shadow-sm'
                    : isDarkMode
                      ? 'text-slate-400 hover:text-slate-200'
                      : 'text-slate-500 hover:text-slate-800'
            ...
            {displayMode === 'itinerary' && itineraryGroups.length > 1 && (
              <div className={`px-4 py-2 border-b ${drawerBorder} shrink-0`}>
                <div className={`flex gap-1 p-1 rounded-xl ${isDarkMode ? 'bg-slate-800' : 'bg-slate-100'}`} role="tablist" aria-label="Itinerary group">
                  {itineraryGroups.map((grp, idx) => (
                    <button
                      ...
                      className={`flex-1 px-3 py-2 rounded-lg text-xs font-semibold transition-all ${activeItineraryIndex === idx
                        ? 'bg-white text-visa-blue shadow-sm'
                        : isDarkMode
                          ? 'text-slate-400 hover:text-slate-200'
                          : 'text-slate-500 hover:text-slate-800'
            ...
            {/* Results Header */}
            <div className={`px-4 py-3 flex items-center justify-between gap-2 border-b ${drawerBorder} shrink-0`}>
              <h3 className={`font-heading font-bold text-xs uppercase tracking-widest ${drawerMuted}`}>
                {displayMode === 'itinerary' ? itineraryGroups[activeItineraryIndex]?.group_name ?? 'Itinerary' : 'Recommended for You'}
              </h3>
              <span
                className={`font-price text-xs font-bold px-2 py-1.5 rounded-md tracking-tighter ${isDarkMode
            ...
            {/* Scrollable Results */}
            <div
              className={`flex-1 overflow-y-auto overflow-x-hidden px-4 py-4 min-h-0 ${isDarkMode ? 'scrollbar-drawer-dark' : 'scrollbar-drawer'
                }`}
            >
              {displayMode === 'single' ? (
                    ...
                    <>
                      ...
                      <div className={`mt-6 pt-6 border-t ${drawerBorder}`}>
                        <button
                          ...
                          className={`font-heading w-full py-4 rounded-xl font-bold text-sm tracking-tight transition-all ${activeGroupItems.every((r) => selectedIds.includes(r.id))
                            ? 'bg-visa-gold text-slate-900 hover:bg-visa-gold/90'
                            : 'bg-visa-blue text-white hover:bg-visa-blue-dark'
                            }`}
                        >
            ...
            {/* Refine Search Input */}
            <div className={`p-4 border-t ${drawerBorder} ${drawerBg} shrink-0`}>
              <div className="relative flex items-center gap-2">
                <input
                  value={chatInput}
                  onChange={(e) => setChatInput(e.target.value)}
